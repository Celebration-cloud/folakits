...
function Log({name}) {
  ...
  console.log(chain)
  const control = [
    <LoginComponent key="login" handleChain={handleChain} handleChainSignUp={handleChainSignUp} setLogged={setLogged}/>,
    <SignUpComponent key="signup" handleChainLogin={handleChainLogin}/>,
    <ForgotPasswordComponent key="forgot-password" />,
    <UserDashboard key="dashboard" name={name} sessions={session}/>,
  ];
  return (
    <div>
      <Nav/>
