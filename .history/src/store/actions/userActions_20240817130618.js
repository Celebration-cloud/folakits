...
export const userRating =
  (userRating) => async () => {
    const addRef = doc(db, "product", "rating");
    const updateRef = query(
      doc(db, "product", "rating"),
      where("name", "==", userRating.name)
    );
    try {
      if (userRating.find((item) => item.name === userRating.name)){
        await updateDoc(updateRef, {
          rating: userRating.rating,
          });
      }else{
        await setDoc(addRef, userRating);
      }
      toaster.success("Rating added")
    } catch (error) {
      const errorCode = error.code;
      const errorMessage = error.message;
      console.log(error);
