...
export const receiveItem= () => async (dispatch, getState) => {
   try {
     ...
     dispatch({ type: ActionTypes.GET_PRODUCTS, payload: error.message });
   }
}
export const getItems = (name) => async (dispatch, getState) => {
  if (name === "Shop" || name === "") return dispatch({ type: ActionTypes.GET_PRODUCTS, payload: newData });
  try {
    const productCollectionRef = collection(db, "products");
...
export const getItemOrders= (userData) => async (dispatch, getState) => {
  const currentState = getState();
  if(currentState.product.products.length < 1) return dispatch({ type: ActionTypes.GET_PRODUCTS, payload: [] });
  try {
