...
export const getItemOrders= (userData) => async (dispatch, getState) => {
    try {
      if (userData === "latest") {
      const productCollectionRef = query(collection(db, "orders"), orderBy("createdAt", "desc"),
      limit(5));
    }
    if (userData === "price") {
      const productCollectionRef = query(collection(db, "orders"), orderBy("price", "desc"))

    }
    if (userData === "lowToHigh") {
     const productCollectionRef = query(collection(db, "orders"), orderBy("price", "asc"))
    }
    if (userData === "highToLow") {
      setFilteredItem(rest.current.sort((a, b) => a.price - b.price));
    }
       const productCollectionRef = query(collection(db, "products"), orderBy("price", "desc"));
