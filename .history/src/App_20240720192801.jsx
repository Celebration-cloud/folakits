...
import { getCategoryItems, getItems, receiveItem, searchItem } from "./store/actions/quoteActions";
...
function App() {
  ...
  console.log(nameRef.current, window.location.pathname.toString().slice(1));
useEffect(() => {
    ...
    try {
      ...
      const unsubscribe = onSnapshot(userCollectionRef, (querySnapshot) => {
        ...
        if (
          ...
          window.location.pathname.toString().slice(1) !== "Men's clothes" ||
          window.location.pathname.toString().slice(1) !== "Jewelry" ||
          window.location.pathname.toString().slice(1) !== "Women's clothes"
        ) {
          dispatch(searchItem(nameRef.current));
        }
        if (
          window.location.pathname === "/Men's%20clothes" ||
          window.location.pathname === "/Women's%20clothes" ||
