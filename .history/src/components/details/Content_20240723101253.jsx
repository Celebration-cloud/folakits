...
function Content({ preview }) {
  ...
  const exist = wishlistRef.current?.find((item) => item.id !== productDetails.id) !==
    undefined;
    const [mark] = useState(wishlistRef.current?.find((item) => item.id !== productDetails.id) !==
    undefined)
