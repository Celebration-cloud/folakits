  ...
  const fet = useCallback(
    async() => {
      ...
      const unsubscribe = onSnapshot(userCollectionRef, (querySnapshot) => {
        ...
        console.log(newData[0]?.WishList);
        dispatch({ type: ActionTypes.GET_USERS, payload: newData });
        dispatch({ type: ActionTypes.GET_CART, payload: newData[0]?.cart ? newData[0]?.cart : []  });
        dispatch({ type: ActionTypes.GET_WISHLIST, payload: newData[0]?.WishList ? newData[0]?.WishList : [] });
        dispatch({ type: ActionTypes.GET_ADDRESS, payload: newData[0]?.address ? newData[0]?.address : []});
        dispatch({ type: ActionTypes.GET_ORDERS, payload: newData[0]?.orders ? newData[0]?.orders : [] });
