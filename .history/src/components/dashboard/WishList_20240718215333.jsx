      ...
      {items.map((item, index) => (
        <div key={index} className={styles.wishItem}>
          ...
          <div className={styles.details}>
            <h3>{item.name}</h3>
            <p style={{color: "blue"}}>{item.originalPrice} {item.originalPrice && `-`} {item.price}</p>
            <p>{item.description}</p>
            <span>Category: <span style={{color: "blue"}}>{item.category}</span></span>
          </div>
          <div className={styles.option}>

          <button onClick={() => Navigate(``)} style={{...removeButtonStyle, color: "white", backgroundColor: "blue"}}>
            Select Options
          </button>
