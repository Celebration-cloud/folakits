...
export default function ProductList() {
  ...
  async function handleDelete(id, product, img) {
    try {
      await deleteDoc(doc(db, "products", id));
      if (img?.length) {
        const productStorageRef = ref(storage, `products/${product}`);
        const metadata = await getMetadata(productStorageRef);
        if (metadata.exists) {
          const res = await listAll(productStorageRef);
          await Promise.all(
            res.items.forEach(async (item) => {
              await deleteObject(ref(storage, `products/${product}/${item.name}`));
            })
          );
        }
      }
        ...
        <>
          ...
          >
            ...
            <DialogActions>
              ...
              <Button
                onClick={() => handleDelete(params.row.id, params.row.product, params.row.img)}
                autoFocus
              >
                Agree
              </Button>
