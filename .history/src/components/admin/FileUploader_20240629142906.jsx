import { FileCard, FileRejectionReason, FileUploader, majorScale, MimeType, Pane, rebaseFiles } from "evergreen-ui";
import React, { useMemo, useRef } from "react";
export default function FileUploaderMultipleUploadDemo() {
    // const acceptedMimeTypes = useRef([])
    useMemo(() => 
            acceptedMimeTypes.current = [MimeType.jpeg, MimeType.pdf]
        , [acceptedMimeTypes])
  const maxFiles = 5;
  ...
  const handleRemove = React.useCallback(
    (file) => {
      ...
      const { accepted, rejected } = rebaseFiles(
        ...
        { acceptedMimeTypes.current, maxFiles, maxSizeInBytes }
      );

      setFiles(accepted);
      setFileRejections(rejected);
    },
    [acceptedMimeTypes.current, files, fileRejections, maxFiles, maxSizeInBytes]
  );
  ...
  return (
    <Pane maxWidth={654}>
      <FileUploader
        acceptedMimeTypes={acceptedMimeTypes.current}
        label="Upload Files"
