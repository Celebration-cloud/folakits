...
function ShopComponent() {
  ...
  const totalPages = Math.ceil(product.current?.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;

  const currentItems = product.current?.slice(startIndex, endIndex);
  ...
  return (
    <div className={styles.container}>
      ...
      <span style={{ marginLeft: "20px", padding: "10px" }}>
        Showing all {product.current?.length} results
      </span>
