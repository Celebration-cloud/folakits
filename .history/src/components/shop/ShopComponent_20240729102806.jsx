...
import { getCategoryItems, searchItem } from "../../store/actions/quoteActions";
...
function ShopComponent() {
  ...
  const [rangePrice, setRangePrice] = useState("range")
  ...
  switch (rangePrice) {
    case "range":
      filtered = productRef.current?.filter((product) => {
 ...
 switch (sortType) {
  case "latest":
    filtered = productRef.current;
    break;
   case "lowToHigh":
     filtered = productRef.current?.slice().sort((a, b) => a.price - b.price);
     break;
   case "highToLow":
     filtered = productRef.current?.slice().sort((a, b) => b.price - a.price);
     break;
   default:
     break;
 }
  ...
  const fet = useCallback(() => {
    if (["Men's clothes", "Women's clothes", "Jewelry"].includes(name)) {
     ...
     if (
       window.location.pathname.slice(1) !== "Shop" &&
       window.location.pathname.slice(1) !== "Men's%20clothes" &&
       window.location.pathname.slice(1) !== "Jewelry" &&
    ...
    <div className={styles.container}>
        ...
        <div className={styles.sort}>
          <FormControl>
            ...
            >
              ...
              <MenuItem value="priceRange">Price: High to Low</MenuItem>
            </Select>
          </FormControl>
        </div>
      ...
      >
        ...
        <div className="offcanvas-body">
          <PriceRangeFilter setPriceRange={setPriceRange} priceRange={priceRange} setRangePrice={setRangePrice}/>
        </div>
      </div>
    </div>
  );
