...
function ShopComponent() {
   ...
   const wishes = userData && userData[0]?.wishlist;
   const dispatch = useDispatch()
   const {name} = useParams()
   const [wishlist, setWishlist] = useState([userData[0]?.wishlist]);
   ...
   useEffect(() => {
     ...
     if(userData){
       ...
       dispatch(
         userWishlist(wishlist, userData[0]?.user_name)
       );
     }
   }, [dispatch, userData, wishlist]);
    useCallback(() => {
      window.scrollTo(0, 0);
      if (userData) {
