...
function ShopComponent() {
  ...
  prduct.current = products;
  // console.log(wishes);

  console.log( products, wishlist, userData), 
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [dispatch]);
  
  // ... (prduct of the component)
  ...
  const totalPages = Math.ceil(prduct.current?.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;

  const currentItems = prduct.current?.slice(startIndex, endIndex);
  ...
  return (
    <div className={styles.container}>
      ...
      <span style={{ marginLeft: "20px", padding: "10px" }}>
        Showing all {prduct.current?.length} results
      </span>
