...
export default function MenuComponent() {
  ...
  const navigate = useNavigate();
    ...
    function handleSubmit(id){
      ...
      const linkDetail= advert.find((item) => {
        return item.id === id
      })
      function handleSubmit(id) {
        const linkDetail = advert.find((item) => item.id === id); // Fixed the return statement
        // navigate(`/detail/${linkDetail.id}`)
    ...
    <div className={styles.menu}>
      ...
      {randomProduct ? (
        <div className={styles.advert}>
          ...
          <button
            onClick={() => handleSubmit(randomProduct?.id)}
