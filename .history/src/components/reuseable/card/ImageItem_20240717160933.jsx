...
function ImageItem({ product, setWishlist, wishlist, wishes, wish }) {
  ...
  function handleWish() {
    ...
    if(userData){
      ...
      // dispatch(
      //   userWishlist(wishlist, userData)
      // );
      dispatch(
        userWishlist(wish.current, userData)
      );
      
    }
    // setWishlist((prev) => [...prev, product]);
    wish.current = [...wish.current, product]
    // userData[0]?.wishlist?.map((item) => {
    //   if (item?.id !== product?.id) {
    //   }
    // });
