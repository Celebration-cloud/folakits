...
function ImageItem({ product, setWishlist }) {
  ...
  function handleWish() {
    ...
    setWishlist((prev) => 
      prev.map(item => item?.id !== product.id ? [...item, product] : [...item.filter(item => item.id === product.id)]) 
    );
    ...
    //   dispatch(
    //     userWishlist(product, userData[0]?.user_name)
    //   );
    // }
  }
  return (
