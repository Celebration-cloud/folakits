...
import {userRatingData} from '../../../store/actions/userActions'
...
const userRating = product?.rating.find(
  (item) => item.name === userData[0]?.user_name
) || null
...
console.log(productRatings, userRating);
...
const handleRatingChange = (event, newValue) => {
  ...
  if (userRating) {
    const updateRate = productRatings.map((item) =>
      item.name === userData[0]?.user_name
        ? { ...item, rating: newValue }
        : item
    );

    setProductRatings(updateRate);
    dispatch(userRatingData(productRatings, product?.product));
     console.log(productRatings, product?.product);
  } else {
    productRatings.push({name: userData[0]?.user_name, rating: newValue})
    console.log(productRatings, product?.product);
     dispatch(userRatingData(productRatings, product?.product));
  }
};
      ...
      if (userData) {
        ...
        if (existingIndex !== -1) {
          ...
          toaster.warning("Item is already in the cart");
            ...
            if (existingIndex !== -1) {
              return toaster.warning("Out of stock")
              ...
              dispatch(
                userCartList(cartListRef.current, userData[0].user_name)
              );
