...
function handleRemoveItem(itemId) {
  ...
  return (
    <>
      ...
      {cart < 1 && (
        <div cal="flex flex-col items-center justify-center h-screen bg-background">
          <img
            src="https://placehold.co/400?text=🛒&bg=primary&text_color=primary-foreground"
            alt="empty-cart"
            cal="mb-8 rounded-lg shadow-lg"
          />
          <p cal="text-primary-foreground text-lg font-semibold">
            Your cart is empty
          </p>
          <p cal="text-secondary-foreground mt-2">
            Start shopping to add items to your cart!
          </p>
          <button cal="mt-4 px-6 py-2 bg-accent text-accent-foreground rounded-md hover:bg-accent/80 transition">
            Shop Now
          </button>
        </div>
      )}
