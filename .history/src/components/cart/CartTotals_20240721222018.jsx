...
const CartTotals = ({ subtotal, shipping , cart, step}) => {
  ...
  return (
    <div className={styles.cartTotals}>
      ...
      {step > 1 && 
        cart?.map((item) => (
          <div key={item.id} className="flex justify-between items-center border-b border-border py-2">
        <div className="flex items-center space-x-2">
          <span className="text-foreground font-semibold">{item.product}</span>
          <span className="bg-muted text-muted-foreground text-xs px-2 py-1 rounded-full">
            × {item.amount}
          </span>
        </div>
        <span className="text-foreground">{item.amount * item.price}</span>
      </div>
        ))
