...
function App() {
  ...
  console.log(nameRef.current, decodeURIComponent(window.location.pathname.slice(1)));
useEffect(() => {
  ...
  async() => {
    try {
      ...
      const unsubscribe = onSnapshot(userCollectionRef, (querySnapshot) => {
        ...
        if (
          detailsRef &&
          decodeURIComponent(window.location.pathname.slice(1)) === detailsRef
        ) {
          dispatch(getItemDetails(detailsRef));
        }
        if (
          decodeURIComponent(window.location.pathname.slice(1)).includes(`Shop/`)
        ) {
          dispatch(getItemDetails(detailsRef &&detailsRef));
        }
